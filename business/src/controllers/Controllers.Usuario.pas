...
begin
  var service := TServiceUsuario.Create;
  var LFoto := Req.Body<TMemoryStream>; // aqui na requisição ele usa binary, o tipo da body...
  try
    if (not service.GetById(req.Params['id']).IsEmpty) then
    begin
      if not Assigned(LFoto) then
        raise EHorseException.Create().Status(THTTPStatus.BadRequest).Error('Foto inválida');
  ...
  try
    ...
    begin
      ...
      if not Assigned(Foto) then
        raise EHorseException.Create().Status(THTTPStatus.NotFound).Error('Não existe foto');
      Res.Send(Foto);
    end
    else
        raise EHorseException.Create.Status(THTTPStatus.BadRequest).Error('Usuario nao cadastrado.');
